...
void dumpWav(T)(T[] samples, uint sampleRate, ushort channels, string filename) {
	dumpWav(samples, sampleRate, channels, File(filename, "w").lockingBinaryWriter);
}

///
void dumpWav(T, R)(T[] samples, uint sampleRate, ushort channels, R file) if (isOutputRange!(R, ubyte)) {
	SampleChunk _;
	dumpWav(samples, sampleRate, channels, file, _);
}

///
void dumpWav(T)(T[] samples, uint sampleRate, ushort channels, string filename, SampleChunk sampleChunk) {
	dumpWav(samples, sampleRate, channels, File(filename, "w").lockingBinaryWriter, sampleChunk);
}

///
void dumpWav(T, R)(T[] samples, uint sampleRate, ushort channels, R file, SampleChunk sampleChunk) if (isOutputRange!(R, ubyte)) {
